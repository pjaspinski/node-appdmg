...
exports.create = function (volname, size, filesystem, cb) {
  temp.template('%s.dmg').writeFile('', function (err, outname) {
    ...
    const outnameWithDate = `${outname}-${Date.now()}`;
    console.log(`Volume name: ${outnameWithDate}`)

    const args = [
      'create', outnameWithDate,
      '-ov',
      '-fs', filesystem || 'HFS+',
      '-size', size,
      '-volname', volname
    ...
    util.sh('hdiutil', args, function (err) {
      if (!err) return cb(null, outnameWithDate);

      fs.unlink(outnameWithDate, () => cb(err));
    })
  })
}
...
exports.attach = function (path, cb) {
  ...
  console.log(`Mounting command: ${["hdiutil", ...args].join(" ")}`)
